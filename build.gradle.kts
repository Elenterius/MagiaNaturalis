...
minecraft {
    ...
    injectedTags.put("MOD_ID", project.property("mod_id")!!)
    injectedTags.put("MOD_NAME", project.property("mod_name")!!)
    injectedTags.put("MOD_VERSION", project.version)
    injectedTags.put("MOD_DEPENDENCIES", project.property("mod_dependencies")!!)
    injectedTags.put("MC_VERSIONS", "[1.7.10]")
    ...
    extraRunJvmArguments.add("-Dfml=${project.property("core_mod_class")!!}")
...
tasks.jar {
    manifest {
        attributes(
            mapOf(
                "FMLCorePlugin" to project.property("core_mod_class")!!,
...
tasks.processResources.configure {
    val projVersion = project.version as String // Needed for configuration cache to work
    inputs.property("version", projVersion)

    filesMatching("mcmod.info") {
        expand(
            mapOf(
                "mod_version" to projVersion,
                "mod_id" to project.property("mod_id"),
                "mod_name" to project.property("mod_name"),
                "mod_description" to project.property("mod_description"),
                "mod_url" to project.property("mod_url"),
                "mod_update_url" to project.property("mod_update_url"),
                ...
                "compiler" {
                    ...
                    afterEvaluate {
                        self.javac.moduleJavacAdditionalOptions = mapOf(
                            (project.name + ".main") to tasks.compileJava.get().options.compilerArgs.map { '"' + it + '"' }.joinToString(" ")
                        )
                    }
                }
            }
