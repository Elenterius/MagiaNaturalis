...
public class PacketHandler extends SimpleChannelInboundHandler<PacketBase>
{
	...
	private void handleBiomeChange(EntityPlayer player, PacketBiomeChange packet)
	{
		...
		{
			WorldUtil.setBiomeAt(Platform.getClientWorld(), packet.x, packet.z, BiomeGenBase.getBiome(packet.biomeID));
			int y = Platform.getClientWorld().getTopSolidOrLiquidBlock(packet.x, packet.z);
			Platform.getClientWorld().markBlockForUpdate(packet.x, y, packet.z); //Update Biome Coloring!!!
		}
	}
