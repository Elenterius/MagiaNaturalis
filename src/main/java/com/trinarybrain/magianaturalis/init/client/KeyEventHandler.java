...
public class KeyEventHandler {
    ...
    public void playerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (event.phase == TickEvent.Phase.START) {
            byte id = 0;
            if (KeyBindings.DECREASE_SIZE_KEY.isPressed()) {
                id = 2;
            }
            else if (KeyBindings.INCREASE_SIZE_KEY.isPressed()) {
                id = 3;
            }
            else if (GuiScreen.isCtrlKeyDown() && KeyBindings.MISC_KEY.isPressed()) {
                id = 4; //Meta
            }
            else if (KeyBindings.MISC_KEY.isPressed()) {
                id = 5; //Shape
            }
            else if (KeyBindings.PICK_BLOCK_KEY.isPressed()) {
